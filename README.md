# OtusLinux_Homework_14.
Что сделано:
- Создан стенд с 2 хостами log и web.
- Написан playbook, который:
    - добавляет в hosts связку имя соседа + ip адрес
    - настраивает лог сервер
    - устанавливает nginx сервер
    - проводит настройку хранения логов 
    - настраивает аудит и отправку логов аудита
Как запустить:
- из директории проекта запустить "vagrant up" для запуска 2 хостов (log и web)
- после создания запустить "ansible-playbook provision_logging.yml"


# OtusLinux_Homework_13.
Что сделано:
- написан playbook для развертывания стенда, на котором с 2 хостов собираются бэкапы на выделенный сервер. Для сбора бэкапов используется BORG.
- написанный playbook:
    - используя наработки с предыдущей лабораторной устанавливает нужные инструменты для анализа и траблшутинга, устанавливает параметры ядра, параметры сети и параметры памяти под работу в качестве сервера (роль initialize)
    - устанавливает borg, создает пользователя borg и директорию запуска скрипта (роль borg_install)
    - реализует доступ до сервера бэкапов по ключам (сбор ключей с клиентов - get_ssh_pub, передача ключей серверу - give_ssh_pub ('command="/path/to/bin/borg serve" - не реализована - доработается на боевых задачах))
    - инициализирует сбор бекапов (директории /etc) с клиентов на сервер бекапов по расписанию (cron). Для этого используется роль borg_init  
Как запустить:
- из директории HW13-Backup запустить "vagrant up" для запуска 3 хостов (2 клиента и 1 сервер)
- после создания запустить "ansible-playbook provision_backup.yml"
